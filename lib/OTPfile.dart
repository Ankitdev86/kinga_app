...
import 'package:kinga/ProfileView.dart';
...
class _VerificationPageState extends State<VerificationPage> {
                    ...
                    child: Container(
                      child: FlatButton(
                        ...
                        child: Center(
                          child: Text(
                            "RESEND",
                            style: TextStyle(fontFamily: "EuclidCircularA-SemiBold", fontSize: 14, letterSpacing: 1, color: Color(0xFF2C51BE)),
                          ),
                        ),
                      ),
                    )),
  ...
  Future<http.Response> registerUser(String url, Map jsonMap, BuildContext context) async {
    var body = json.encode(jsonMap);
    var responseInternet;
    try {
      responseInternet = await http.post(Uri.parse(url), headers: {"Content-Type": "application/json"}, body: body).then((http.Response response) {
        ...
        if (statusCode < 200 || statusCode > 400 || json == null) {
          ...
          if (responseData.status == "1") {
            ...
            // SHDFClass.saveSharedPrefValueBoolean(AppConstants.Session, true);
            setState(() {});
            Alert(context: context, title: "SUCCESS", desc: "Signup Successfully.", image: Image.asset("Assets/success.png"), buttons: [
              DialogButton(
                child: Text(
                  "OK",
                  style: TextStyle(color: Colors.white, fontSize: 16),
...
class OTPDigitTextFieldBox extends StatelessWidget {
  ...
  const OTPDigitTextFieldBox({Key key, @required this.first, @required this.last}) : super(key: key);
